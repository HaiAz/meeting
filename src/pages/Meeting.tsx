import { Box, Heading, Flex, Button, Icon, type ButtonProps } from "@chakra-ui/react"
...
const clampVolume = (v?: number) => (v == null || Number.isNaN(v) ? 1 : Math.max(0, Math.min(1, v)))
...
const TOKENS: Record<"admin" | "abc" | "xyz", string> = {
  ...
  abc: "",
...
export default function MeetingPage() {
  ...
  // store
  const { setSlot, clearSlot, upsertUsers, removeUsers, slots, resetAll } = useRoomStore()
  ...
  }, [engine, setParticipants, upsertUsers, removeUsers, setSlot, clearSlot, userID])
  ...
  const onJoin = useCallback(async () => {
    ...
    setIsJoining(true)
    try {
      const ok = await loginRoom(engine, roomID, TOKENS[tokenKey], { userID, userName })
  ...
  const handleCamera = useCallback(async () => {
    ...
    if (localCam) {
      await stopCamera(engine, localCam, camPubId, camBoxes.current.get(userID) ?? null)
      setLocalCam(null)
      setCamPubId("")
    } else {
      const { stream, streamId } = await startCamera(engine, { userID, quality: 3 })
      setLocalCam(stream)
      ...
      if (audioBox) {
        if (uid === userID) {
          ...
          if (!audioId) {
            ...
            if (item?.stream) {
              mountMediaToBox(audioBox, item.stream, { muted: false, volume: 1 })
            } else {
              unmountBox(audioBox)
            }
          }
  ...
  const showCam = (uid: string) => (uid === userID ? !!localCam : !!slots[uid]?.cam)
  const showScreen = (uid: string) => (uid === userID ? !!localScreen : !!slots[uid]?.screen)
  const showAudio = (uid: string) => (uid === userID ? !!localAudio : !!slots[uid]?.audio)
    ...
    <Box p={4} position="relative">
      ...
      >
        ...
        <Flex gap={3}>
          ...
          <Button {...buttonProps} onClick={handleShareScreen} disabled={!isJoined}>
